...
public class MySQLiteHelper extends SQLiteOpenHelper {
    ...
    public Cursor getAllStockData( long reference)
    {
        SQLiteDatabase db= this.getWritableDatabase();
        Cursor result = db.rawQuery("select T.ID as _id, D.Nom, T.Reference, C.Stock from "+categorie.TABLE_NAME+" C, "
                +tube_volant.TABLE_NAME+ " T, " +distributeur.TABLE_NAME+
    ...
    public void ajoutData(SQLiteDatabase db){
        ...
        Distributeur d = new Distributeur("ASHAWAY","8 rue des acacias","contact",457896575L,"lo52@utbm.fr");
        this.insertDistributeurData(db,d);
        this.insertDistributeurData(db,new Distributeur("ALTEN","9 rue des acacias","contact2",607789575L,"lo53@utbm.fr"));
        this.insertCategorieData(db,new Categorie(56,1));
        this.insertCategorieData(db,new Categorie(26,1));
        this.insertCategorieData(db,new Categorie(120,2));
        //db.insertCategorieData();
        this.insertTube_VolantData(db,new Tube_Volant(3333,196,"autonne","2017",1));
        this.insertTube_VolantData(db,new Tube_Volant(26,200,"printemps","2017",2));
        this.insertTube_VolantData(db,new Tube_Volant(22,29,"hiver","2017",2));
        this.insertTube_VolantData(db,new Tube_Volant(21,1000,"ete","2017",1));
        this.insertTube_VolantData(db,new Tube_Volant(666,1050,"autonne","2018",3));
        //db.insertTube_VolantData();
        //db.insertTube_VolantData();
        //db.insertTube_VolantData();*/
    }
    ...
    public void ajoutAchatData(SQLiteDatabase db){
        ...
        this.insertClientData(db,new Client("Stark","TONY","sport","les forges",987452163));
        this.insertClientData(db,new Client("Star","LORD","Galaxy","avengers",89454));
        this.insertFactureData(db,new Facture(1,12,true,2));
        this.insertFactureData(db,new Facture(2,26,false,1));
        this.insertFactureData(db,new Facture(1,8,true,4));
        //db.insertCategorieData();
        //db.insertTube_VolantData();
        //db.insertTube_VolantData();
        //db.insertTube_VolantData();
